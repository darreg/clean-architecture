...
func NewOrderRepository(tx *adapter.Transactor, db *sql.DB) *OrderRepository {
	return &OrderRepository{tx: tx, db: db}
}
...
func (o OrderRepository) Remove(ctx context.Context, number string) error {
	_, err := o.tx.ExecContext(ctx, "DELETE FROM orders WHERE number=$1", number)
	return err
}
